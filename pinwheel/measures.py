...
def get_spikes(N=10, t_start=500, t_stop = 2000, path='Result',pop=""):
  ...
  for i in xrange(N):
    for j in xrange(N):
      print "%s/%s_Bloc_%d_%d.gdf" %(path,pop, i, j)
...
def big_diag_get_r(path,idm1,idm2):
   mr=np.zeros([idm1,idm2])
   stdr=np.zeros([idm1,idm2])
   cvs=np.zeros([idm1,idm2])

   params=cp.load(open("pinwheel.par"))   
   for i1 in range(idm1):
      for i2 in range(idm2):
        s=get_spikes(params["diag"]["N"],0,1000,"Results/"+path+"_%s_%s"%(1.0*i1,1.0*i2),"E")   
        mrate,srate=get_rates_m_std(s,5)
        cv=get_cvs(s)
        mr[i1,i2]=mrate.mean()
        stdr[i1,i2]=srate.mean() 
        cvs[i1,i2]=cv.mean() 
   np.save(path+"/",mr)
   np.save(path+"/",stdr)
   np.save(path+"/",cvs)
