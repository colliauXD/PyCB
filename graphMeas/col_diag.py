...
import conn_graph as cg

sim.nest.Install("mymodule")

global  seed
rng      = sim.NumpyRNG(seed=249856, parallel_safe=False)
seed     = 512468
...
class Column(object):  
  ...
  def _record(self, variable='spikes', to_file=True):
    ...
    if variable is 'spikes_I':
      ...
      res = sim.nest.Create('multimeter', params={'record_from': ["V_m"],
                                              'withgid'    : False, 
                                              'withtime'   : False, 
                                              #'to_file'    : True, 
                                              #'to_memory'  : True, 
      ...
      sim.nest.DivergentConnect(res, self.exc.all_cells.tolist(), model='static_synapse')
  ...
  def _print(self, variable, file, gather=True):
    ...
    if variable is 'spikes_I':
      ...
      lfp = sim.nest.GetStatus(self.mean_recorder)[0]["events"]["V_m"]
...
class LGN(object):
  def __init__(self, N):
    self.N   = N
    self.LGN = sim.Population((N, N), sim.SpikeSourceArray, {'spike_times':[]})

  def mpi_print(self, string):
    if sim.simulator.state.mpi_rank == 0: print string    
  ...
  def _connect_cell2pop(self, cell, targets, layer, factor=10):
    ...
    d      = sim.RandomDistribution('uniform', [timestep, 30*timestep], rng=rng)
    weight = sim.RandomDistribution('normal', (factor*layer.w_E, layer.w_E/3), boundaries=(0, layer.w_E*100), constrain='redraw', rng=rng)
    cell   = self.N*cell[0]+cell[1]
    c1     = MyConnector(numpy.repeat(weights, layer.Ne+layer.Ni), weights=weight, delays=d)
    prj1   = sim.Projection(self.LGN[[cell]], layer.get_all_from_blocks(indices), c1, target='excitatory', rng=rng)
    return prj1.size()
...
class Layer(object):
  ...
  def get_stim_idx(self):
    return range(self.ext_stim.first_id, self.ext_stim.last_id+1) 
  ...
  def stim_n_sources(self, stimulus, idx, n_source):
    ...
    prj     = sim.Projection(self.ext_stim.sample(n_source), target_cells, conn, target='excitatory', rng=rng)
  ...
  def simulate(self, simtime, print_rates=True):
    ...
    os.system('rm -rf *.gdf')
    if sim.simulator.state.mpi_rank == 0:
      sim.nest.SetStatus([0], {'print_time':True})
    run(simtime)
    if print_rates:
      firing_rate = 0
      for bloc in self.blocs.flatten():
  ...
  def _connect_blocs(self, source, targets, synapse='exc'):
    ...
    if synapse is 'exc':
      ...
      weight = sim.RandomDistribution('normal', (self.w_E, self.w_E/3), boundaries=(0, self.w_E*100), constrain='redraw', rng=rng)
      c1     = MyConnector(numpy.repeat(factor, self.Ne+self.Ni), weights=weight, delays=d) 
      prj1   = sim.Projection(source, self.get_all_from_blocks(indices), c1, target='excitatory', rng=rng)
    elif synapse is 'inh':
      source = self.blocs[source].inh
      factor = self.li * weights
      weight = sim.RandomDistribution('normal', (self.w_I, self.w_I/3), boundaries=(0, self.w_I*100), constrain='redraw', rng=rng)
      c1     = MyConnector(numpy.repeat(factor, self.Ne+self.Ni), weights=weight, delays=d)
      prj1   = sim.Projection(source, self.get_all_from_blocks(indices), c1, target='inhibitory', rng=rng)
    result = prj1.size()
    return result

  def connect_single_col(self, idx, col_params):
     conn_EE=MyConnector(numpy.repeat(col_params['ee'],self.Ne),
                         weights=col_params['wE'],delays=self.timestep)
     prjEE=sim.Projection(self.blocs[idx[0]].exc,self.blocs[idx[0]].exc,
                      conn_EE,target="excitatory",rng=rng)

     conn_EI=MyConnector(numpy.repeat(col_params['ei'],self.Ni),
                         weights=col_params['wE'],delays=self.timestep)
     prjEI=sim.Projection(self.blocs[idx[0]].exc,self.blocs[idx[0]].inh,
                      conn_EI, target="excitatory",rng=rng)

     conn_IE=MyConnector(numpy.repeat(col_params['ie'],self.Ne),
                         weights=col_params['wI'],delays=self.timestep)
     prjIE=sim.Projection(self.blocs[idx[0]].inh,self.blocs[idx[0]].exc,
                      conn_IE, target="inhibitory",rng=rng)

     conn_II=MyConnector(numpy.repeat(col_params['ii'], self.Ni),
                 weights=col_params['wI'],delays=self.timestep)
     prjII=sim.Projection(self.blocs[idx[0]].inh, self.blocs[idx[0]].inh,
                      conn_II,target="inhibitory", rng=rng)
     if idx==self.graph: self.conn_graph.conn_col(conn_EE.edg,conn_EI.edg,conn_IE.edg,conn_II.edg)
...
def generic_launch_network(path,params,verbose=True):
  ...
  if not os.path.exists(path):
      os.system('mkdir %s' % path)
  #Build Layer 
  network = Layer(params["diag"]["N"], params['cell_params'],params['col'], params['sim'], True,le=0, li=0, velocity=None)
  ...
  for idx in network._indices():
    ...
    print "with", params["stim"]["rate_E"], "Hz sources"
    ...
    params["cell_params"]["a"]=As[idx[0]]
    params["cell_params"]["b"]=Bs[idx[1]]
    set(network.get_exc_from_blocks([idx]),param={'a': params["cell_params"]["a"],'b': params["cell_params"]["b"]})
...
network = Layer(params["diag"]["N"], params['cell_params'],params['col'], params['sim'],graph=[(0,0)], le=0, li=0, velocity=None)
network.stimulate_on_target(params["stim"],[(0,0)])
r=network.connect_single_col([(0,0)], params['col'])
network.conn_graph.get_deg("in")
#ax=network.conn_graph.init_plot("out")
network.conn_graph.plot_it("in_deg.pdf")

"""
s=mea.get_spikes(params["diag"]["N"], 500, 1000, path,"E")
