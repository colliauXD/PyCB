...
class Layer(object):
  ...
  def stimulate(self, stimulus):
    if (stimulus['type'] == 'Poisson_E'):
      ...
      #conn    = MyConnector(numpy.repeat(stimulus['ee'],self.Ne+self.Ni),weights = stimulus['wE'], delays=self.timestep)
      conn    = MyConnector(numpy.repeat(stimulus['ee'],self.Ne),weights = stimulus['wE'], delays=self.timestep)
      prj     = Projection(self.ext_stim, self.get_exc_from_blocks(indices), conn, target='excitatory', rng=rng)	     	
      self.mpi_print("--> %d excitatory connections were established from external sources ..." %prj.size())


    if stimulus['type'] == 'centered_poisson':
      self.ext_stim = Population(1, native_cell_type('poisson_generator'), {'rate': stimulus['rate'], 'stop' : float(stimulus['duration'])})
      indices = [] 
      for i in range(14, 16):
         for j in range(14, 16):
            indices += [(i, j)]
      weight  = RandomDistribution('normal', (self.w_E, self.w_E/3), boundaries=(0, self.w_E*100), constrain='redraw', rng=rng)
      conn    = FastAllToAllConnector(weights=weight, delays=self.timestep)   
      prj     = Projection(self.ext_stim, self.get_all_from_blocks(indices), conn, target='excitatory', rng=rng)
      self.mpi_print("--> %d excitatory connections were established from external sources ..." %prj.size())
...
def generic_launch_network(path,params,verbose=True):
  ...
  network = Layer(params["sim"]["N"], params['cell_params'],params['col'], params['sim'],le=0, li=0, velocity=None)
  network.stimulate(params['stim'])  
  for i in range(params["sim"]["N"]):
      for j in range(params["sim"]["N"]):
         r=network.connect_single_col((i,j), params['col'])
  print r, "internal conn"
